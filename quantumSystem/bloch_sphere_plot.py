...
def convert_to_bloch_vector(rho):
    ...
    ax = np.trace(rho @ pauli_basis[1].numpy()).real
    ay = np.trace(rho @ pauli_basis[2].numpy()).real
    az = np.trace(rho @ pauli_basis[3].numpy()).real
    return [ax, ay, az]
...
for _ in range(samples):
    this_system = pureState(1)
    this_system.haarRandomize()
    bloch_vectors.append(this_system.density_matrix.numpy())

bloch_vectors = np.array([convert_to_bloch_vector(s) for s in bloch_vectors])
plot_bloch_sphere(bloch_vectors)
